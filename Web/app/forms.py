...
class LaptopPredictionForm(forms.Form):
    ...
    ram_capacity = forms.ChoiceField(choices=RAM_CAPACITY_CHOICES, widget=forms.Select(attrs={'class': 'form-control select2'}), label="Ram capacity(GB)" )
    ...
    hard_drive_capacity = forms.ChoiceField(choices=HARD_DRIVE_CAPACITY_CHOICES, widget=forms.Select(attrs={'class': 'form-control select2'}), label="Hard drive capacity(GB)")
